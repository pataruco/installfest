...
name: content-design
description: >
  Apply content design principles to user-facing content — reviewing, critiquing, and generating
  microcopy, page content, error messages, form labels, notifications, onboarding flows, empty states,
  and any other UI or service content. Use this skill whenever the user mentions content design,
  UX writing, microcopy, UI copy, content critique, content review, plain language review, readability,
  content patterns, or asks to write/improve any user-facing text (error messages, button labels,
  form hints, confirmation screens, empty states, notification copy, onboarding flows, help text,
  accessibility copy). Also trigger when the user asks to simplify or clarify written content,
  apply plain English principles, review content against user needs, or when they reference
  GOV.UK content standards, Sarah Richards, or content design as a discipline. Even if they just say
  "this copy feels off" or "can you make this clearer" — that's content design work, use this skill.
---
...
1. **What the content is trying to do** — state the apparent user need and task context
2. **What's working** — don't skip this, it anchors the discussion
3. **Issues** — for each issue, explain the problem, why it matters for the user, and provide a rewrite
4. **Rewritten version** — the full revised content, ready to use
...
2. A short note on assumptions about user need and context (so the user can correct you)
...
1. **Start with user needs** — what does someone need to know or do at this point?
2. **Front-load** — the most important information comes first, always
3. **Use plain language** — if a simpler word works, use it. This is not dumbing down, it's opening up
4. **One thing per sentence** — short sentences with one idea each
5. **Active voice** — say who does what. "We'll send you an email" not "An email will be sent"
6. **Be specific** — "Takes about 5 minutes" not "This is a quick process"
7. **Remove what doesn't help** — every word must earn its place
8. **Design for scanning** — people don't read, they scan. Structure matters
9. **Consider the whole journey** — content doesn't exist in isolation, it sits within a flow
10. **Accessibility is not optional** — plain language, clear structure, and good labelling are accessibility
